...
use bevy::transform::helper::ComputeGlobalTransformError::MissingTransform;
...
pub fn setup_gun_base(mut commands: Commands) {
    ...
    let transform = Transform::from_translation(Vec3::new(0., y, 2.));
    commands
        .spawn(sprite)
        .insert(transform)
...
pub fn setup_gun_barrel(mut commands: Commands) {
    ...
    let sprite = Sprite {
        color: Color::srgb(0.32, 0.36, 0.41),
        custom_size: Some(sprite_size),
        ..default()
    };
    let transform = Transform::from_translation(Vec3::new(0., y, 1.));
    commands
        .spawn(sprite)
        .insert(transform)
...
fn move_gun(
    ...
    let gamepad_right = button_inputs.pressed(GamepadButton::DPadRight);
    let gamepad_left = button_inputs.pressed(GamepadButton::DPadLeft);
    let any_left = keyboard_left || gamepad_left;
    let any_right = keyboard_right || gamepad_right;

    // Rotate the gun
    let (mut velocity, transform) = query.single_mut().expect("Gun entity not found!");
    let (gun_axis, gun_angle) = transform.rotation.to_axis_angle();
    velocity.angular = if any_left && any_right {
        0.
...
fn gun_bomb_collision_system(
    mut event_reader: MessageReader<CollisionEvent>,
    mut event_writer: MessageWriter<GunExplosionEvent>,
    ...
    let (gun_entity, gun_transform) = gun_query.single().expect("No gun entity.");
    let (gun_mount_entity, gun_mount_transform) =
        gun_mount_query.single().expect("No gun mount entity.");
    let (gun_base_entity, _gun_base_transform) =
        gun_base_query.single().expect("No gun base entity.");
    ...
    for &collision_event in event_reader.read() {
        if let CollisionEvent::Started(entity1, entity2, _flags) = collision_event {
            ...
            if is_bomb_collision && is_gun_collision {
                event_writer.write(GunExplosionEvent {
...
fn stop_gun(mut gun_query: Query<&mut Velocity, With<Gun>>) {
    let mut velocity = gun_query.single_mut().expect("Gun velocity not found.");
    velocity.angular = 0.;
}
